        ...
        <div className="absolute z-10 p-4 mt-2 min-w-48 border rounded shadow-lg bg-white">
          ...
          {topCandidates.map((cand) => (
            <div
              ...
              className={`py-1 px-2 text-left text-md ${
                cand.isSelected ? "font-semibold" : ""
              } whitespace-nowrap `}
              style={{
...
const TranslationApp: React.FC = () => {
  ...
  const handleCopyToClipboard = () => {
    if (translationTokens.length > 0) {
      const translatedText = translationTokens.map((t) => t.token).join(", "); // 翻訳結果のテキストを結合
      navigator.clipboard
        .writeText(translatedText)
        .then(() => {
          alert("クリップボードにコピーしました！"); // 簡単なフィードバック
          // より洗練されたフィードバックを実装することもできます
        })
        .catch((err) => {
                ...
                {translationTokens.map((t, index) => (
                  <>
                    <TokenWithTooltip
                      ...
                      vocab={vocab}
                    />
                    {index < translationTokens.length - 1 && <span>,</span>}
                  </>
                ))}
